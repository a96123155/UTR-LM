# python Finetune_extract_append_predictor_CellLine_10fold-lr-huber-nrepeats.py --cell_line Muscle --prefix 11M.4.1.1.2 --epochs 2 --folds 10 --modelfile /home/ubuntu/esm2/saved_models/ESM2_1.5_five_species_TrainLossMin_6layers_16heads_256embedsize_4096batchToks.pkl --cnn_layers 0 --avg_emb --patience 0  --huber_loss --finetune --label_type te_log

# nohup python Finetune_extract_append_predictor_CellLine_10fold-lr-huber-nrepeats.py --cell_line Muscle --prefix 11M.4.1.1.2 --epochs 150 --folds 10 --modelfile /home/ubuntu/esm2/saved_models/ESM2_1.5_five_species_TrainLossMin_6layers_16heads_256embedsize_4096batchToks.pkl --cnn_layers 0 --avg_emb --patience 50  --huber_loss --finetune --label_type te_log > /home/ubuntu/esm2/Cao/outputs/11M.4.1.1.2.patience100.out 2>&1 &


# nohup python Finetune_extract_append_predictor_CellLine_10fold-lr-huber-nrepeats.py --cell_line pc3 --prefix 11P.4.1.1.2 --epochs 150 --folds 10 --modelfile /home/ubuntu/esm2/saved_models/ESM2_1.5_five_species_TrainLossMin_6layers_16heads_256embedsize_4096batchToks.pkl --cnn_layers 0 --avg_emb --patience 50  --huber_loss --finetune --label_type te_log > /home/ubuntu/esm2/Cao/outputs/11P.4.1.1.2.patience100.out 2>&1 &


# nohup python Finetune_extract_append_predictor_CellLine_10fold-lr-huber-nrepeats.py --cell_line HEK --prefix 11H.4.1.1.2 --epochs 150 --folds 10 --modelfile /home/ubuntu/esm2/saved_models/ESM2_1.5_five_species_TrainLossMin_6layers_16heads_256embedsize_4096batchToks.pkl --cnn_layers 0 --avg_emb --patience 50  --huber_loss --finetune --label_type te_log > /home/ubuntu/esm2/Cao/outputs/11H.4.1.1.2.patience100.out 2>&1 &

# nohup python Finetune_extract_append_predictor_CellLine_10fold-lr-huber-nrepeats.py --cell_line MuscleLR100 --prefix 11M.LR100.4.1.1.2 --epochs 150 --folds 10 --modelfile /home/ubuntu/esm2/saved_models/ESM2_1.5_five_species_TrainLossMin_6layers_16heads_256embedsize_4096batchToks.pkl --cnn_layers 0 --avg_emb --patience 50  --huber_loss --finetune --label_type te_median --seq_type utr_lr100 --inp_len 200 > /home/ubuntu/esm2/Cao/outputs/11M.LR100.1.1.2.patience100.out 2>&1 &
...
prefix = f'mathfeatureCVESM2lr1e-5_{args.prefix}'
...
def train_step(train_dataloader, model, epoch, batches, feats):
#     print(feats.shape,'---')
    ...
    for i, (labels, strs, masked_strs, toks, masked_toks, _) in enumerate(train_dataloader):
        toks = toks.to(device)
        labels = torch.FloatTensor(labels).to(device).reshape(-1, 1)
        batch = batches[i]
        feats_batch = torch.Tensor(feats[batch]).to(device)
#         print(feats_batch.shape)
        outputs, _ = model(toks, feats_batch, return_representation = True, return_contacts=True)
...
def eval_step(test_dataloader, model, epoch, batches, feats):
    model.eval()
    y_pred_list, y_true_list, loss_list = [], [], []
    strs_list = []
    with torch.no_grad():
        for i, (labels, strs, masked_strs, toks, masked_toks, _) in enumerate(test_dataloader):
...
for repeat in range(args.repeats):
    ...
    print('====Load Data====')
    all_data = pd.read_csv(f'/home/ubuntu/5UTR_Optimizer-master/data/{cell_line}_sequence_with_mathfeature.csv', index_col = 0)
    n_feats = all_data.shape[1] - 18 # muscle = 786
    ...
    for fold, (train_index, val_index) in enumerate(KF.split(data)):
        ...
        imputer = imputer.fit(e_train.iloc[:, -n_feats:])
        e_train_feats = imputer.transform(e_train.iloc[:, -n_feats:])
        e_val_feats = imputer.transform(e_val.iloc[:, -n_feats:])

        scaler = StandardScaler()
        scaler.fit(e_train_feats)
        e_train_feats = scaler.transform(e_train_feats)
        e_val_feats = scaler.transform(e_val_feats)
        n_feats = e_train_feats.shape[1]
        ...
        for epoch in trange(1, args.epochs + 1):
            ...
            metrics_train, loss_train, _ = train_step(train_dataloader, model, epoch, train_batches, e_train_feats)
            ...
            if args.epochs >= args.patience:
                if metrics_val[0] > r2_best: 
                    path_saver = f'/home/ubuntu/esm2/Cao/saved_models/{filename}_repeat{repeat}_fold{fold}.pkl'
        ...
        metrics_train, loss_train, train_pred = eval_step(train_dataloader, model_best, ep_best, train_batches, e_train_feats)
        metrics_val, loss_val, val_pred = eval_step(val_dataloader, model_best, ep_best, val_batches, e_val_feats)
        ...
        print('====Save y_pred====')
        val_pred.to_csv(f'/home/ubuntu/esm2/Cao/y_pred/{filename}_valdata_repeat{repeat}_fold{fold}.csv', index = False)    
        val_atg_pred.to_csv(f'/home/ubuntu/esm2/Cao/y_pred/{filename}_valdata_atg_repeat{repeat}_fold{fold}.csv', index = False)
        val_n_atg_pred.to_csv(f'/home/ubuntu/esm2/Cao/y_pred/{filename}_valdata_noatg_repeat{repeat}_fold{fold}.csv', index = False)


        print('====Save Metrics====')
        metrics_repeat_dict[f'Repeat{repeat}-Fold{fold}'] = metrics_train + metrics_train_atg + metrics_train_n_atg + metrics_val + metrics_val_atg + metrics_val_n_atg + [loss_train.tolist()] + [loss_val.tolist()]   
        ...
        axes[fold, 6].plot(range(args.epochs), loss_train_list, label = f'Fold_Train_{fold} = {loss_train:.4f} (Best={ep_best})', linestyle = ':')
        axes[fold, 6].plot(range(args.epochs), loss_val_list, label = f'Fold_Val_{fold} = {loss_val:.4f} (Best={ep_best})')
        axes[fold, 6].legend(fontsize = 12)

        plt.savefig(f'/home/ubuntu/esm2/Cao/figures/{filename}_repeat{repeat}.tif')
