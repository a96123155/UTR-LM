...
print('====Load Data====')
train_data = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/{args.train_file}')
if args.train_atg: 
...
#         hidden_visualization_dict[epoch] = hidden_visualization
        ...
        ##### results
            ...
            if epoch == 1 or epoch % args.log_interval == 0 or epoch == args.init_epochs+1 or epoch >= args.init_epochs + args.epochs:
                print('=====Generate results=====')

                train_dataset, train_dataloader = generate_dataset_dataloader(e_train, args.label_type)
                test_dataset, test_dataloader = generate_dataset_dataloader(e_val, args.label_type)
                ...
                train_atg_pred = e_train_pred[e_train_pred[args.seq_type].str.contains('ATG')]
                e_val_atg_pred = e_val_pred[e_val_pred[args.seq_type].str.contains('ATG')]
                ...
                if len(e_val_atg_pred) != 0:
                    metrics_val_atg = performances(e_val_atg_pred[args.label_type], e_val_atg_pred['y_pred'])
                else:
                    metrics_val_atg = [np.nan, np.nan, np.nan]
                    
                
                train_n_atg_pred = e_train_pred[~e_train_pred[args.seq_type].str.contains('ATG')]
                e_val_n_atg_pred = e_val_pred[~e_val_pred[args.seq_type].str.contains('ATG')]
                if len(train_n_atg_pred) != 0:
                ...
                if len(e_val_n_atg_pred) != 0:
                    metrics_val_n_atg = performances(e_val_n_atg_pred[args.label_type], e_val_n_atg_pred['y_pred'])
                else:
                    metrics_val_n_atg = [np.nan, np.nan, np.nan]


                print('====Save y_pred====')
                e_train_pred.to_csv(f'/home/ubuntu/esm2/Sample/y_pred/{filename}_train_fold{i}.csv', index = False)
                e_val_pred.to_csv(f'/home/ubuntu/esm2/Sample/y_pred/{filename}_val_fold{i}.csv', index = False)
                ...
                loss_train_repeat_dict[i], loss_val_repeat_dict[i] = loss_train_list, loss_val_list #epoch
                ...
                axes[0, 3].scatter(e_val_pred[args.label_type], e_val_pred.y_pred)

                axes[1, 0].hist(train_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
                axes[1, 0].hist(train_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
                axes[1, 0].set_title(f'All Train ATG Data | {args.label_type} | n = {len(train_atg_pred)} | R2 = {metrics_train_atg[0]:.4f}', fontsize = 12)
                axes[1, 0].legend(fontsize = 12)

                axes[1, 1].hist(e_val_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
                axes[1, 1].hist(e_val_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
                axes[1, 1].set_title(f'All val ATG Data | {args.label_type} | n = {len(e_val_atg_pred)} | R2 = {metrics_val_atg[0]:.4f}', fontsize = 12)
                axes[1, 1].legend(fontsize = 12)

                axes[1, 2].hist(train_n_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
                axes[1, 2].hist(train_n_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
                axes[1, 2].set_title(f'All Train No-ATG Data | {args.label_type} | n = {len(train_n_atg_pred)} | R2 = {metrics_train_n_atg[0]:.4f}', fontsize = 12)
                axes[1, 2].legend(fontsize = 12)

                axes[1, 3].hist(e_val_n_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
                axes[1, 3].hist(e_val_n_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
                axes[1, 3].set_title(f'All val No-ATG Data | {args.label_type} | n = {len(e_val_n_atg_pred)} | R2 = {metrics_val_n_atg[0]:.4f}', fontsize = 12)
