...
parser.add_argument('--seq_type', type=str, default = 'utr_50')
...
print('====Load Data====')
data = pd.read_csv(f'/home/ubuntu/Experimental_Data/{args.train_file}')
if args.train_atg: 
...
#                                  eps = 1e-08)
    ...
    loss_best, ep_best, spearman_best = np.inf, -1, -1
    ...
    for epoch in trange(args.init_epochs+1, args.init_epochs + args.epochs + 2):
        ...
        if args.local_rank == 0:
            ...
            if args.epochs >= args.patience:
                if metrics_val[-1] > spearman_best: 
                    path_saver = f'/home/ubuntu/Experimental_Data/saved_models/{filename}_fold{i}.pt' # _epoch{epoch}
                    spearman_best, ep_best = metrics_val[-1], epoch
                    
                    torch.save(model.eval().state_dict(), path_saver) # 
                    print(f'****Saving model in {path_saver}: Best epoch = {ep_best} | Train Loss = {loss_train:.4f} |  Val Loss = {loss_val:.4f} | spearman_best = {spearman_best:.4f}')
                    model_best = deepcopy(model)
        ...
        ##### results
            ...
            if epoch % args.log_interval == 0:
                ...
                print('====Save Metrics====')
                metrics_repeat_dict[i] = metrics_train + metrics_val
...
print(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_val_fold{i}.csv')
