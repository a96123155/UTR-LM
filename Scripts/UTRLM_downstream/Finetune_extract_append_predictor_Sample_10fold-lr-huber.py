...
parser.add_argument('--prefix', type=str, default = '9.1')
...
parser.add_argument('--train_file', type = str, default = '4.13_train_data_GSM3130441_mcherry_1.csv')
...
for i, (train_index, val_index) in enumerate(KF.split(data)):
    ...
    train_dataset, train_dataloader = generate_dataset_dataloader(e_train, train_obj_col)
...
#         hidden_visualization_dict[epoch] = hidden_visualization
        ...
        if args.epochs >= args.patience:
            if metrics_val[0] > r2_best: 
                path_saver = f'/home/ubuntu/esm2/Sample/saved_models/{filename}_fold{i}_epoch{epoch}.pkl'
    ...
    original_train_data = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/{original_train_file}')
    ...
    traindata_atg_pred = traindata_pred[traindata_pred.utr.str.contains('ATG')]
    e_test_atg_pred = e_test_pred[e_test_pred.utr.str.contains('ATG')]
    ...
    traindata_n_atg_pred = traindata_pred[~traindata_pred.utr.str.contains('ATG')]
    e_test_n_atg_pred = e_test_pred[~e_test_pred.utr.str.contains('ATG')]
    ...
    loss_train_repeat_dict[i], loss_val_repeat_dict[i], loss_test_repeat_dict[i] = loss_train_list, loss_val_list, [loss_test]*args.epochs
    ...
    axes[0, 2].plot(range(args.epochs), loss_train_repeat_dict[i], label = f'MSELoss: Fold_Train_{i}')
    axes[0, 3].plot(range(args.epochs), loss_val_repeat_dict[i], label = f'MSELoss: Fold_Val_{i}')
    axes[1, 3].plot(range(args.epochs), loss_train_repeat_dict[i], label = f'MSELoss: Fold_Train_{i} = {loss_train:.4f} (Best = {ep_best})', linestyle = ':')
    axes[1, 3].plot(range(args.epochs), loss_val_repeat_dict[i], label = f'MSELoss: Fold_Val_{i} = {loss_val:.4f} (Best = {ep_best})')
    axes[1, 3].plot(range(args.epochs), loss_test_repeat_dict[i], label = f'MSELoss: All_Test_{i} = {loss_test:.4f} (Best = {ep_best})', linestyle = '--')
