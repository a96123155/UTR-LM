# python Finetune_extract_append_predictor_RVAC_10fold.py --prefix 9.1 --epochs 1 --folds 2 --cnn_layers 1 --train_file 4.1_train_data_GSM3130435_egfp_unmod_1.csv --modelfile /home/ubuntu/saved_models/ESM2_1.4_five_species_TrainLossMin_6layers_16heads_128embedsize_4096batchToks.pkl --finetune --scaler --log2 --avg_emb
# python -m torch.distributed.launch --nproc_per_node=2 Finetune_extract_append_predictor_RVAC_10fold-lr-huber-DDP.py

# /home/ubuntu/5UTR_Optimizer-master/data/Muscle_sequence.csv
# netstat -ntlp | grep 1234
# nohup python -m torch.distributed.launch --nproc_per_node=3 --master_port 1234 Finetune_extract_append_predictor_RVAC_10fold-lr-huber-DDP.py --prefix DDP9.1 --lr 0.1 --huber_loss --train_file 4.1_train_data_GSM3130435_egfp_unmod_1.csv --modelfile /home/ubuntu/saved_models/ESM2SI_3.4_fiveSpeciesCao_6layers_16heads_128embedsize_4096batchToks_MLMLossMin.pkl --scaler --bos_emb --train_n_atg --device_ids 0,1,2 --finetune --test1fold --epochs 300 > /home/ubuntu/RVAC/outputs/DDP9.1.out 2>&1 &


# nohup python -m torch.distributed.launch --nproc_per_node=4 --master_port 1234 Finetune_extract_append_predictor_RVAC_10fold-lr-huber-DDP.py --prefix DDP9.1 --huber_loss --train_file 4.1_train_data_GSM3130435_egfp_unmod_1.csv  --scaler --bos_emb --train_n_atg --device_ids 0,1,2,3 --finetune --test1fold --load_wholemodel --modelfile /home/ubuntu/saved_models/ESM2SI_3.4_fiveSpeciesCao_6layers_16heads_128embedsize_4096batchToks_MLMLossMin.pkl --load_wholemodel --finetune_modelfile /home/ubuntu/RVAC/saved_models/CVESM2lr1e-5_DDP9.1_unmod_1_10folds_rl_LabelScalerTrue_LabelLog2False_AvgEmbFalse_BosEmbTrue_CNNlayer0_epoch300_nodes40_dropout30.2_finetuneTrue_huberlossTrue_magicFalse_lr0.01_fold0_epoch72_lr0.01.pt --epochs 300 --init_epochs 72 --lr 0.001 --log_interval 50 > /home/ubuntu/RVAC/outputs/DDP9.1.out 2>&1 &
...
parser.add_argument('--prefix', type=str, default = 'RVACv1')
...
parser.add_argument('--train_file', type = str, default = 'v1_Time16_Dosage125_Processed_experimental_data.csv')
...
parser.add_argument('--modelfile', type = str, default = '/home/ubuntu/saved_models/ESM2_1.4_five_species_TrainLossMin_6layers_16heads_128embedsize_4096batchToks.pkl')

parser.add_argument('--finetune_modelfile', type = str, default = '/home/ubuntu/saved_models/CVESM2lr1e-5_DDP9.1_unmod_1_10folds_rl_LabelScalerTrue_LabelLog2False_AvgEmbFalse_BosEmbTrue_CNNlayer0_epoch300_nodes40_dropout30.2_finetuneTrue_huberlossTrue_magicFalse_fold0_epoch19_lr0.1.pt')
...
def train_step(data, batches_loader, model):
    ...
    for i, batch in tqdm(enumerate(train_batches_loader)):
        ...
        dataset = FastaBatchedDataset(e_data.loc[:, train_obj_col], e_data[args.seq_type], mask_prob = 0.0)
...
def eval_step(dataloader, model, epoch, data = None):
    ...
    with torch.no_grad():
        ...
        print(f'max(y_true_list) = {max(y_true_list):.4f}, max(y_pred_list) = {max(y_pred_list):.4f}')
        print(f'min(y_true_list) = {min(y_true_list):.4f}, min(y_pred_list) = {min(y_pred_list):.4f}')
        e_pred = pd.DataFrame([strs_list, y_true_list, y_pred_list], index = [args.seq_type, 'y_true', 'y_pred']).T
        
        if data is not None: 
            data_pred = pd.merge(e_pred, data, on = [args.seq_type])
        else:
            data_pred = e_pred
...
print('====Load Data====')
rvac_data = pd.read_csv(f'/home/ubuntu/Experimental_Data/{args.train_file}')
sample_data = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/4.1_train_data_GSM3130435_egfp_unmod_1.csv')
rvac_data['rl'] = rvac_data['label']
rvac_data['utr'] = rvac_data['utr_50']
rvac_data['type'] = 'RVAC'
sample_data['type'] = 'Sample'
# data = pd.concat([rvac_data, sample_data], axis = 0).reset_index(drop = True)


# if args.train_atg: 
#     data = data[data[args.seq_type].str.contains('ATG')]
...
for i, (train_index, val_index) in enumerate(KF.split(rvac_data)):
    print(f'====Begin Train Fold = {i}====')
    e_train_rvac = rvac_data.iloc[val_index, :]
    e_val = rvac_data.iloc[train_index, :]
    e_train = pd.concat([e_train_rvac, sample_data], axis = 0).sample(frac = 1).reset_index(drop = True)
    print(f'RVAC={rvac_data.shape}|Sample={sample_data.shape}|train_rvac={len(train_index)}|test_rvac={len(val_index)}|train={len(e_train)}|test={len(e_val)}')
...
#         hidden_visualization_dict[epoch] = hidden_visualization
        ...
        if args.local_rank == 0:
            ...
            if args.epochs >= args.patience:
                if metrics_val[0] > r2_best: 
                    path_saver = f'/home/ubuntu/Experimental_Data/saved_models/{filename}_fold{i}_epoch{epoch}.pt'
        ...
        #         model_best = torch.load(path_saver)#, map_location = lambda storage, loc : storage.cuda(0)))
                ...
                metrics_train, loss_train, train_pred = eval_step(train_dataloader, model_best, ep_best, e_train)
                metrics_val, loss_val, val_pred = eval_step(val_dataloader, model_best, ep_best, e_val)
                
                print('====ATG Analysis====')
                try:
                    train_atg_pred = train_pred[train_pred.utr_originial_varylength.str.contains('ATG')]
                    val_atg_pred = val_pred[val_pred.utr_originial_varylength.str.contains('ATG')]
                    if len(train_atg_pred) != 0:
                        metrics_train_atg = performances(train_atg_pred[args.label_type], train_atg_pred['y_pred'])
                    else:
                        metrics_train_atg = [np.nan, np.nan, np.nan]

                    if len(val_atg_pred) != 0:
                        metrics_val_atg = performances(val_atg_pred[args.label_type], val_atg_pred['y_pred'])
                    else:
                        metrics_val_atg = [np.nan, np.nan, np.nan]
                except:
                    metrics_train_atg = [np.nan, np.nan, np.nan]
                    metrics_val_atg = [np.nan, np.nan, np.nan]
                
                try:
                    train_n_atg_pred = train_pred[~train_pred.utr_originial_varylength.str.contains('ATG')]
                    val_n_atg_pred = val_pred[~val_pred.utr_originial_varylength.str.contains('ATG')]
                    metrics_train_n_atg = performances(train_n_atg_pred[args.label_type], train_n_atg_pred['y_pred'])
                    metrics_val_n_atg = performances(val_n_atg_pred[args.label_type], val_n_atg_pred['y_pred'])
                except:
                    metrics_train_n_atg = [np.nan, np.nan, np.nan]
                    metrics_val_n_atg = [np.nan, np.nan, np.nan]

                print('====Save y_pred====')
                train_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_train_fold{i}.csv', index = False)
                val_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_val_fold{i}.csv', index = False)

#                 train_atg_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_train_atg_fold{i}.csv', index = False)
#                 train_n_atg_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_train_noatg_fold{i}.csv', index = False)

#                 val_atg_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_val_atg_fold{i}.csv', index = False)
#                 val_n_atg_pred.to_csv(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_val_noatg_fold{i}.csv', index = False)

                print('====Save Metrics====')
                metrics_repeat_dict[i] = metrics_train + metrics_val + metrics_train_atg + metrics_val_atg + metrics_train_n_atg + metrics_val_n_atg
                ...
                metrics_repeat_df.to_csv(f'/home/ubuntu/Experimental_Data/results/{filename}_metrics.csv', index = True)

                loss_train_repeat_dict[i], loss_val_repeat_dict[i] = loss_train_list, loss_val_list

                ##### Figures

#                 fig, axes = plt.subplots(nrows = 4, ncols = 2, figsize = (10, 15))

#                 axes[0, 0].hist(train_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[0, 0].hist(train_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[0, 0].set_title(f'Fold Train Data | {args.label_type} | n = {len(train_pred)} | R2 = {metrics_train[0]:.4f}', fontsize = 12)
#                 axes[0, 0].legend(fontsize = 12)

#                 axes[0, 1].hist(val_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[0, 1].hist(val_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[0, 1].set_title(f'Fold Val Data | {args.label_type} | n = {len(val_pred)} | R2 = {metrics_val[0]:.4f}', fontsize = 12)
#                 axes[0, 1].legend(fontsize = 12)

#                 axes[2, 0].hist(train_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[2, 0].hist(train_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[2, 0].set_title(f'Fold Train ATG Data | {args.label_type} | n = {len(train_atg_pred)} | R2 = {metrics_train_atg[0]:.4f}', fontsize = 12)
#                 axes[2, 0].legend(fontsize = 12)

#                 axes[2, 1].hist(val_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[2, 1].hist(val_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[2, 1].set_title(f'Fold Val ATG Data | {args.label_type} | n = {len(val_atg_pred)} | R2 = {metrics_val_atg[0]:.4f}', fontsize = 12)
#                 axes[2, 1].legend(fontsize = 12)

#                 axes[3, 0].hist(train_n_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[3, 0].hist(train_n_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[3, 0].set_title(f'Fold Train No-ATG Data | {args.label_type} | n = {len(train_n_atg_pred)} | R2 = {metrics_train_n_atg[0]:.4f}', fontsize = 12)
#                 axes[3, 0].legend(fontsize = 12)

#                 axes[3, 1].hist(val_n_atg_pred[args.label_type], bins = 100, alpha = 0.3, label = f'Y_True')
#                 axes[3, 1].hist(val_n_atg_pred.y_pred, bins = 100, alpha = 0.3, label = f'Y_Pred')
#                 axes[3, 1].set_title(f'Fold Val No-ATG Data | {args.label_type} | n = {len(val_n_atg_pred)} | R2 = {metrics_val_n_atg[0]:.4f}', fontsize = 12)
#                 axes[3, 1].legend(fontsize = 12)

#                 axes[4, 0].plot(range(args.init_epochs, epoch), loss_train_repeat_dict[i], label = f'MSELoss: Fold_Train_{i}')
#                 axes[4, 1].plot(range(args.init_epochs, epoch), loss_val_repeat_dict[i], label = f'MSELoss: Fold_Val_{i}')
                
#                 axes[4, 0].legend(fontsize = 12)
#                 axes[4, 1].legend(fontsize = 12)
#                 plt.savefig(f'/home/ubuntu/Experimental_Data/figures/{filename}_fold{i}.tif')
    if args.test1fold: break
    best_epoch_list.append(ep_best)
print(f'/home/ubuntu/Experimental_Data/saved_models/{filename}_fold{i}.pkl')    
print(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_train_fold{i}.csv')
print(f'/home/ubuntu/Experimental_Data/y_pred/{filename}_test_fold{i}.csv')
print(f'/home/ubuntu/Experimental_Data/results/{filename}_metrics.csv')    
print(f'/home/ubuntu/Experimental_Data/figures/{filename}_fold{i}.tif')
