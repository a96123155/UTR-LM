...
parser.add_argument('--prefix', type=str, default = 'no10foldcvBenchmarkSplit')
...
print('====Load Data====')
e_train = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/VaryLengthRandomTrain_sequence.csv')
e_val = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/VaryLengthRandomTest_sequence.csv')
test_human_data = pd.read_csv(f'/home/ubuntu/human_5utr_modeling-master/data/VaryLengthHumanTest_sequence.csv')

e_train = e_train.sample(frac=1, random_state = seed).reset_index(drop=True)
...
print('-----', e_train.shape, e_val.shape, test_human_data.shape)
...
for epoch in trange(args.init_epochs+1, args.init_epochs + args.epochs + 2):
    ...
    if args.local_rank == 0:
        ...
        if args.epochs >= args.patience:
            if metrics_val[-1] > spearman_best: 
                path_saver = f'/home/ubuntu/esm2/Sample/saved_models/{filename}_epoch{epoch}.pt'
    ...
    ##### results
        ...
        if epoch == 1 or epoch % args.log_interval == 0 or epoch == args.init_epochs+1 or epoch >= args.init_epochs + args.epochs:
            ...
            test_random_dataset, test_random_dataloader = generate_dataset_dataloader(e_val, args.label_type)
            ...
            metrics_random, loss_random, e_random_pred = eval_step(test_random_dataloader, model_best, ep_best, e_val)
            metrics_human, loss_human, e_human_pred = eval_step(test_human_dataloader, model_best, ep_best, test_human_data)


            print('====Save y_pred====')
            e_train_pred.to_csv(f'/home/ubuntu/esm2/Sample/y_pred/{filename}_train.csv', index = False)
            e_random_pred.to_csv(f'/home/ubuntu/esm2/Sample/y_pred/{filename}_testrandom.csv', index = False)
            e_human_pred.to_csv(f'/home/ubuntu/esm2/Sample/y_pred/{filename}_testhuman.csv', index = False)

            print('====Save Metrics====')
            metrics_repeat_df = pd.DataFrame(metrics_random + metrics_human + metrics_train, 
                                            index = ['Test_Random_R2', 'Test_Random_PearsonR', 'Test_Random_SpearmanR',
                                                    'Test_Human_R2', 'Test_Human_PearsonR', 'Test_Human_SpearmanR',
