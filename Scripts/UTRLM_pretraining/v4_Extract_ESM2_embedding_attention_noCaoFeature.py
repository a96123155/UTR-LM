...
output_dir = f'/home/ubuntu/esm2/Sample/Embedding_Contacts'
ESM2_results_outfilename = args.esm2_modelfile.split('/')[-1].replace('.pkl', '').replace('.pt', '')
filename = args.predictor_modelfile.replace('.pkl', '').replace('.pt', '')
fold_info = ''
...
finetuned_outfilename = f'Finetuned_{cell_line}__{"_".join(filename.split("/")[-1].split("_")[:10])}{fold_info}'#__{ESM2_results_outfilename}'
print('***', filename, fold_info)
...
#             print(results_finetuned_ESM2.keys())
        ...
        data = pd.DataFrame([strs_finetuned_ESM2, labels_finetuned_ESM2, y_pred_list, y_true_list], index = ['seq', 'label', 'y_pred', 'y_true']).T
#         if data is not None: 
#             try:
...
if args.finetune: # args.data_type == IRES Sample
    ...
    predict_model.load_state_dict({k.replace('module.', ''):v for k,v in torch.load(modelfile, map_location=device).items()})
...
#     i = 0
    with torch.no_grad():
        for (labels, strs, masked_strs, toks, masked_toks, _) in tqdm(dataloader):
            ...
            toks = toks.to(device)
            results_nonfinetuned_ESM2 = ESM2_model(toks, repr_layers=repr_layers, need_head_weights=True, return_contacts=True, return_representation=True, return_attentions_symm = args.attentions_symm, return_attentions = args.attentions)
