...
inp_len = [50, 100][args.data_type == 'Cao']
mask_prob = 0.0

modelfile = args.predictor_modelfile
output_dir = f'/home/ubuntu/esm2/Cao/Embedding_Contacts'
...
finetuned_outfilename = f'Finetuned_{cell_line}__{filename}__{ESM2_results_outfilename}'.replace('_6layers_16heads_128embedsize_4096batchToks_MLMLossMin', '').replace('_huberlossTrue_magicFalse', '').replace('_AvgEmbFalse', '')
print(filename)
...
scaler = [False, True]['_caoScaler' in modelfile]
cao_lm = [False, True]['_caoLM' in modelfile]
cao_bm = [False, True]['_caoBM' in modelfile]
esm_bm = [False, True]['_esmBM' in modelfile]
cao_esm_bm = [False, True]['_caoesmBM' in modelfile]
cao_esm_lm = [False, True]['_caoesmLM' in modelfile]
...
class CNN_linear(nn.Module):
    ...
    def forward(self, tokens, features, need_head_weights=True, return_contacts=True, return_representation = True, return_attentions_symm = False, return_attentions = False):
        ...
        x = self.flatten(x_o)
        if cao_lm: features = self.cao_lm(features)
        if cao_bm and len(features) > 1: features = self.cao_bm(features)
        if esm_bm and len(features) > 1: x = self.esm_bm(x)
        x = torch.cat((x, features), 1)
        if cao_esm_bm and len(features) > 1: x = self.cao_esm_bm(x)
        if cao_esm_lm: x = self.cao_esm_lm(x)
...
if args.data_type == 'Sample':
    ...
    data = pd.read_csv(args.data)
#     print(data[args.seq_type].str[-100:].shape)
#     print([len(a) for a in data[args.seq_type].str[-100:][:10]])
    data.rename(columns = {'te_log':'label'}, inplace = True)
    dataset = FastaBatchedDataset(data.loc[:,'label'], data[args.seq_type].str[-100:], mask_prob = mask_prob)
    
#     if args.scaler:
#         scaler = StandardScaler()
#         scaler.fit(e_train.iloc[:, -n_feats:])
#         e_train_feats = scaler.transform(e_train.iloc[:, -n_feats:])
feats = np.array(data.iloc[:, -n_feats:]) 
...
def finetuned_eval_step(test_dataloader, model, epoch, batches, feature, data = None):
    model.eval()
    ...
    with torch.no_grad():
        for i, (labels, strs, masked_strs, toks, masked_toks, _) in enumerate(tqdm(test_dataloader)):
            ...
            outputs, results_finetuned_ESM2,_ = model(toks, feats_batch, need_head_weights=True, return_contacts=True, return_representation=True, return_attentions_symm = args.attentions_symm, return_attentions = args.attentions)
...
if args.finetune: # args.data_type == IRES Sample
    ...
    metrics, data = finetuned_eval_step(dataloader, predict_model, epoch, batches, feats, data)
...
#     i = 0
    with torch.no_grad():
        for (labels, strs, masked_strs, toks, masked_toks, _) in tqdm(dataloader):
            ...
            toks = toks.to(device)
            results_nonfinetuned_ESM2 = ESM2_model(toks, repr_layers=repr_layers, need_head_weights=True, return_contacts=args.contacts, return_representation=True, return_attentions_symm = args.attentions_symm, return_attentions = args.attentions)
